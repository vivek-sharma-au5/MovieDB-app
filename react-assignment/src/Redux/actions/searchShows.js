var pageNumber = 0;
export function searchShows(searchQuery) {
  pageNumber++;
  console.log("counter", pageNumber, searchQuery);
  return (dispatch) => {
    var data = fetch(
      `https://api.themoviedb.org/3/search/tv?api_key=b991ec3735c1278ddf57a1911fea22f3&page=${pageNumber}&query=${searchQuery}`
    );
    data.then((res) => {
      data = res.json();
